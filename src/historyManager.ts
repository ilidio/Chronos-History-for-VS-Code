...
export class HistoryManager {
    ...
    public async showDailyBriefingForDate() {
        if (!this.aiService.isConfigured()) {
            vscode.window.showErrorMessage('Chronos: AI feature is not configured. Please add your Gemini API Key in Settings.', 'Open Settings').then(selection => {
    ...
    private async onSave(doc: vscode.TextDocument) {
        ...
        try {
            ...
            if (history.length > 0) {
                ...
                diff = await this.gitService.getDiff(prevPath, doc.fileName);
